...
use sea_orm::{
    entity, ActiveModelTrait, ColumnTrait, Condition, DatabaseConnection, DatabaseTransaction,
    EntityTrait, Iden, IntoActiveModel, Iterable, ModelTrait, PrimaryKeyToColumn, QueryFilter,
    Related, RelationDef,
};
...
pub fn to_table_name(to_tbl: sea_orm::sea_query::types::TableRef) -> String {
    match to_tbl {
        sea_orm::sea_query::types::TableRef::Table(name) => name.to_string().to_upper_camel_case(),
        sea_orm::sea_query::types::TableRef::SchemaTable(_, name) => {
            name.to_string().to_upper_camel_case()
        }
        sea_orm::sea_query::types::TableRef::DatabaseSchemaTable(_, _, name) => {
            name.to_string().to_upper_camel_case()
        }
        sea_orm::sea_query::types::TableRef::TableAlias(_, name) => {
            name.to_string().to_upper_camel_case()
        }
        sea_orm::sea_query::types::TableRef::SchemaTableAlias(_, _, name) => {
            name.to_string().to_upper_camel_case()
        }
        sea_orm::sea_query::types::TableRef::DatabaseSchemaTableAlias(_, _, _, name) => {
            name.to_string().to_upper_camel_case()
        }
        sea_orm::sea_query::types::TableRef::SubQuery(_, name) => {
            name.to_string().to_upper_camel_case()
        }
        sea_orm::sea_query::types::TableRef::ValuesList(_, name) => {
            name.to_string().to_upper_camel_case()
        }
        sea_orm::sea_query::types::TableRef::FunctionCall(_, name) => {
            name.to_string().to_upper_camel_case()
        }
    }
}
